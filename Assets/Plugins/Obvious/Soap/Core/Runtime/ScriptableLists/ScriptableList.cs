    ...
    public abstract class ScriptableList<T> : ScriptableListBase, IList<T>, IDrawObjectsInInspector
    {
        ...
        public T this[int index]
        {
            ...
            set
            {
                ...
                _itemCounts[oldItem]--;
                if (_itemCounts[oldItem] == 0)
                    _itemCounts.Remove(oldItem);
                OnItemRemoved?.Invoke(oldItem);
                TryAddItemToDictionary(value);
                RaiseItemAddedEvents(value);
            }
        }
        ...
        /// Raises OnItemsRemoved (if items were present), OnItemsAdded (if new items provided), and Modified once.
        /// </summary>
        public void SetItems(IEnumerable<T> items)
        {
            ...
            if (oldItems.Length > 0)
                OnItemsRemoved?.Invoke(oldItems);
            if (newItems.Length > 0)
                OnItemsAdded?.Invoke(newItems);
            Modified?.Invoke();
#if UNITY_EDITOR
