...
namespace Sisus.Init
{
	...
	/// <see cref="Create.Instance{TScriptableObject, TFirstArgument, TSecondArgument, TThirdArgument}">created</see>
	/// or <see cref="InstantiateExtensions.Instantiate{TScriptableObject, TFirstArgument, TSecondArgument, TThirdArgument}">instantiated</see>
	/// with three arguments passed to the <see cref="Init"/> method of the created instance.
	/// <para>
	...
	/// then an <see cref="ScriptableObjectInitializer{TScriptableObject, TFirstArgument, TSecondArgument, TThirdArgument}"/>
	...
	public abstract class ScriptableObject<TFirstArgument, TSecondArgument, TThirdArgument> : ScriptableObject, IInitializable<TFirstArgument, TSecondArgument, TThirdArgument>, IInitializable
		#if UNITY_EDITOR
		...
		#endif
	{
		...
		private protected void Awake()
		{
			...
			else if(InitArgs.TryGet(Context.Awake, this, out TFirstArgument firstArgument, out TSecondArgument secondArgument, out TThirdArgument thirdArgument))
			{
				#if DEBUG || INIT_ARGS_SAFE_MODE
				initState = InitState.Initializing;
				HandleValidate(firstArgument, secondArgument, thirdArgument);
		...
		private void OnValidate()
		{
			...
			{
				Debug.LogWarning($"Initializer {initializer.GetType().Name} is attached to {GetType().Name} at '{AssetDatabase.GetAssetOrScenePath(this)}' but it does not implement IInitializer<{GetType().Name}, {typeof(TFirstArgument).Name}, {typeof(TSecondArgument)}, {typeof(TThirdArgument)}>.", this);
				return;
			}
		...
		internal void InitInternal(TFirstArgument firstArgument, TSecondArgument secondArgument, TThirdArgument thirdArgument)
		{
			#if DEBUG || INIT_ARGS_SAFE_MODE
			initState = InitState.Initializing;
			HandleValidate(firstArgument, secondArgument, thirdArgument);
