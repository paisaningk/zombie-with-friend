...
namespace Sisus.Init
{
	...
	/// then an <see cref="ScriptableObjectInitializer{TScriptableObject, TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument, TFifthArgument, TSixthArgument}"/>
		...
		#endif
	{
		...
		string EditorOnly.IInitializableEditorOnly.NullGuardFailedMessage { get; set; } = "";
		#endif
		...
		/// <param name="fourthArgument"> The fourth object that this <see cref="ScriptableObject"/> depends on. </param>
		/// <param name="fifthArgument"> The fifth object that this <see cref="ScriptableObject"/> depends on. </param>
		/// <param name="sixthArgument"> The sixth object that this <see cref="ScriptableObject"/> depends on. </param>
		...
		protected object this[[DisallowNull] string memberName]
		{
			set
			{
				...
				Inject<ScriptableObject<TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument, TFifthArgument, TSixthArgument>, TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument, TFifthArgument, TSixthArgument>(this, memberName, value);
			}
		}
		...
		bool IInitializable.HasInitializer => initializer;

		bool IInitializable.Init(Context context)
		{
			#if UNITY_EDITOR
			if(initState != InitState.Uninitialized && !context.IsEditMode())
			{
				return true;
			}
			#endif
			...
			if(InitArgs.TryGet(context, this, out TFirstArgument firstArgument, out TSecondArgument secondArgument, out TThirdArgument thirdArgument, out TFourthArgument fourthArgument, out TFifthArgument fifthArgument, out TSixthArgument sixthArgument))
			{
				#if DEBUG || INIT_ARGS_SAFE_MODE
				...
				initState = context.IsEditMode() ? InitState.Uninitialized : InitState.Failed;
				throw; }
				#endif

				#if DEBUG || INIT_ARGS_SAFE_MODE
				initState = context.IsEditMode() ? InitState.Uninitialized : InitState.Initialized;
				#endif
			}
		...
		private void Reset()
		{
			if(InitArgs.TryGet(Context.Reset, this, out TFirstArgument firstArgument, out TSecondArgument secondArgument, out TThirdArgument thirdArgument, out TFourthArgument fourthArgument, out TFifthArgument fifthArgument, out TSixthArgument sixthArgument))
			{
				#if DEBUG || INIT_ARGS_SAFE_MODE
				initState = InitState.Initializing;
				#endif
		...
		private protected void Awake()
		{
			...
			else if(InitArgs.TryGet(Context.Awake, this, out TFirstArgument firstArgument, out TSecondArgument secondArgument, out TThirdArgument thirdArgument, out TFourthArgument fourthArgument, out TFifthArgument fifthArgument, out TSixthArgument sixthArgument))
			{
				#if DEBUG || INIT_ARGS_SAFE_MODE
				initState = InitState.Initializing;
				HandleValidate(firstArgument, secondArgument, thirdArgument, fourthArgument, fifthArgument, sixthArgument);
		...
		private void OnValidate()
		{
			...
			{
				Debug.LogWarning($"Initializer {initializer.GetType().Name} is attached to {GetType().Name} at '{AssetDatabase.GetAssetOrScenePath(this)}' but it does not implement IInitializer<{GetType().Name}, {typeof(TFirstArgument).Name}, {typeof(TSecondArgument)}, {typeof(TThirdArgument)}, {typeof(TFourthArgument)}, {typeof(TFifthArgument)}, {typeof(TSixthArgument)}>.", this);
				return;
			}
			...
			if(!initializersGenericArguments.Any(args => typeof(TFifthArgument).IsAssignableFrom(args[5])))
			{
				Debug.LogWarning($"Initializer {initializer.GetType().Name} of {GetType().Name} at '{AssetDatabase.GetAssetOrScenePath(this)}' invalid argument type {initializersGenericArguments.First()[5].Name}: not assignable to {typeof(TFifthArgument).Name}.", this);
			}

			if(!initializersGenericArguments.Any(args => typeof(TSixthArgument).IsAssignableFrom(args[6])))
			{
				Debug.LogWarning($"Initializer {initializer.GetType().Name} of {GetType().Name} at '{AssetDatabase.GetAssetOrScenePath(this)}' invalid argument type {initializersGenericArguments.First()[6].Name}: not assignable to {typeof(TSixthArgument).Name}.", this);
			}
		}
		#endif
		...
		/// <inheritdoc/>
		void IInitializable<TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument, TFifthArgument, TSixthArgument>.Init(TFirstArgument firstArgument, TSecondArgument secondArgument, TThirdArgument thirdArgument, TFourthArgument fourthArgument, TFifthArgument fifthArgument, TSixthArgument sixthArgument)
		{
			#if DEBUG || INIT_ARGS_SAFE_MODE
			initState = InitState.Initializing;
			HandleValidate(firstArgument, secondArgument, thirdArgument, fourthArgument, fifthArgument, sixthArgument);
		...
		protected virtual void ValidateArguments(TFirstArgument firstArgument, TSecondArgument secondArgument, TThirdArgument thirdArgument, TFourthArgument fourthArgument, TFifthArgument fifthArgument, TSixthArgument sixthArgument)
		{
			#if DEBUG || INIT_ARGS_SAFE_MODE
