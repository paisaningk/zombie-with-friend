...
namespace Sisus.Init
{
	...
	/// <see cref="Create.Instance{TScriptableObject, TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument, TFifthArgument}">created</see>
	/// or <see cref="InstantiateExtensions.Instantiate{TScriptableObject, TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument, TFifthArgument}">instantiated</see>
	/// with five arguments passed to the <see cref="Init"/> method of the created instance.
	/// <para>
	...
	/// then an <see cref="ScriptableObjectInitializer{TScriptableObject, TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument, TFifthArgument}"/>
	/// can be used to specify its initialization arguments.
	/// </para>
	/// <para>
	/// Instances of classes inheriting from <see cref="ScriptableObject{TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument, TFifthArgument}"/>
	...
	public abstract class ScriptableObject<TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument, TFifthArgument>
		: ScriptableObject, IInitializable<TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument, TFifthArgument>, IInitializable
		#if UNITY_EDITOR
		...
		#endif
	{
		...
		protected object this[[DisallowNull] string memberName]
		{
			set
			{
				...
				Inject<ScriptableObject<TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument, TFifthArgument>, TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument, TFifthArgument>(this, memberName, value);
			}
		}
		...
		bool IInitializable.Init(Context context)
		{
			...
			if(InitArgs.TryGet(context, this, out TFirstArgument firstArgument, out TSecondArgument secondArgument, out TThirdArgument thirdArgument, out TFourthArgument fourthArgument, out TFifthArgument fifthArgument))
			{
				...
				HandleValidate(firstArgument, secondArgument, thirdArgument, fourthArgument, fifthArgument);
				#endif
		...
		private protected void Awake()
		{
			...
			else if(InitArgs.TryGet(Context.Awake, this, out TFirstArgument firstArgument, out TSecondArgument secondArgument, out TThirdArgument thirdArgument, out TFourthArgument fourthArgument, out TFifthArgument fifthArgument))
			{
				#if DEBUG || INIT_ARGS_SAFE_MODE
				initState = InitState.Initializing;
				HandleValidate(firstArgument, secondArgument, thirdArgument, fourthArgument, fifthArgument);
			...
			#if !UNITY_EDITOR
			else if(!ServiceInjector.ServicesAreReady)
			{
				ServiceInjector.ServicesBecameReady += OnServicesServicesBecameReady;
				return;

				void OnServicesServicesBecameReady()
				{
					ServiceInjector.ServicesBecameReady -= OnServicesServicesBecameReady;
					Awake();
				}
		...
		private void OnValidate()
		{
			...
			{
				Debug.LogWarning($"Initializer {initializer.GetType().Name} is attached to {GetType().Name} at '{AssetDatabase.GetAssetOrScenePath(this)}' but it does not implement IInitializer<{GetType().Name}, {typeof(TFirstArgument).Name}, {typeof(TSecondArgument)}, {typeof(TThirdArgument)}, {typeof(TFourthArgument)}, {typeof(TFifthArgument)}>.", this);
				return;
			}
		...
		[Conditional("DEBUG"), Conditional("INIT_ARGS_SAFE_MODE"), MethodImpl(MethodImplOptions.AggressiveInlining)]
		private void HandleValidate(TFirstArgument firstArgument, TSecondArgument secondArgument, TThirdArgument thirdArgument, TFourthArgument fourthArgument, TFifthArgument fifthArgument)
		{
			#if DEBUG || INIT_ARGS_SAFE_MODE
			#if UNITY_EDITOR
