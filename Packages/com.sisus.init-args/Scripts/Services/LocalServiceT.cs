...
{
	/// <summary>
	/// Class that can be used to register local services.
	/// </summary>
	/// <remarks>
	/// Local services are bound to a specific <see cref="GameObject"/> and their accessibility
	/// can be limited to a specific set of <see cref="Clients">clients</see> in the scene hierarchy.
	/// </remarks>
	...
	internal static class LocalService<TService>
	{
		#if UNITY_EDITOR
		...
		internal static bool ExistsFor(Component client)
		{
			foreach(var instance in Instances)
			{
				...
				if(Service.IsAccessibleTo(instance, client.transform))
				{
					return instance.serviceProvider is not null || instance.service is not null;
				}
			}
		...
		/// </summary>
		internal static bool ExistsForAllClients()
		{
			foreach(var instance in Instances)
			{
				#if UNITY_EDITOR
				...
				if(instance.clients is not Clients.Everywhere)
				{
					continue;
				}

				if(instance.serviceProvider is not null || instance.service is not null)
				{
					return true;
				}
			}
		...
		public static bool Add([DisallowNull] TService service, Clients clients, [DisallowNull] Component registerer)
		{
			...
			Debug.Log("LocalService<" + Internal.TypeUtility.ToString(typeof(TService)) + ">.Add(" + Internal.TypeUtility.ToString(service.GetType()) + ", " + clients + ", " + registerer + ")");
			#endif
		...
		public static bool Add([DisallowNull] ServiceProvider<TService> serviceProvider, Clients clients, [DisallowNull] Component registerer)
		{
			...
			Debug.Log("LocalService<" + Internal.TypeUtility.ToString(typeof(TService)) + ">.Add(" + Internal.TypeUtility.ToString(serviceProvider?.GetType()) + ", " + clients + ", " + registerer + ")");
			#endif
		...
		{
			#if DEV_MODE && DEBUG_SERVICE_PROVIDERS
			Debug.Log("LocalService<" + Internal.TypeUtility.ToString(typeof(TService)) + ">.Remove(" + Internal.TypeUtility.ToString(serviceProvider?.GetType()) + ", " + registerer + ")");
			#endif
