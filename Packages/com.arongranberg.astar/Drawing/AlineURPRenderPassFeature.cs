            ...
            public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
            {
                ...
                using (IRasterRenderGraphBuilder builder = renderGraph.AddRasterRenderPass<PassData>("ALINE", out PassData passData, profilingSampler))
                {
                    ...
                    builder.SetRenderFunc<PassData>((PassData data, RasterGraphContext context) =>
                        {
                            DrawingManager.instance.ExecuteCustomRenderGraphPass(
                                new DrawingData.CommandBufferWrapper { cmd2 = context.cmd, allowDisablingWireframe = data.allowDisablingWireframe }, data.camera
                            );
                        }
                    );
                }
